...
export function PerformanceDashboard({ isVisible, onToggle }: PerformanceDashboardProps) {
  const [metrics, setMetrics] = useState<PerformanceMetrics>(getPerformanceData())
  ...
  const formatValue = (value: number | undefined, unit: string = 'ms'): string => {
    if (value === undefined) return 'N/A'
    return `${value.toFixed(2)}${unit}`
  }

  const getRating = (value: number | undefined, thresholds: [number, number]): 'good' | 'needs-improvement' | 'poor' | 'unknown' => {
      ...
      {isVisible && (
        <div className="fixed top-16 right-4 z-40 bg-white border border-gray-200 rounded-lg shadow-xl p-4 w-96 max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            {performanceItems.map(({ name, key, icon: Icon, description, thresholds, unit }) => {
